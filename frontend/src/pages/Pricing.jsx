...
export default function Pricing({ initialProductId, clearInitialProductId }) {
  ...
  async function loadPricingRules() {
    try {
      ...
      const settings = Object.fromEntries(
        (response.data || []).map((item) => [item.key, item.value])
      );

      setData((current) => ({
        ...current,
        desiredMargin:
          settings.default_margin !== undefined
            ? settings.default_margin
            : current.desiredMargin,
        taxPercent:
          settings.default_tax !== undefined
            ? settings.default_tax
            : current.taxPercent,
        adsPercent:
          settings.default_ads !== undefined
            ? settings.default_ads
            : current.adsPercent,
        affiliatePercent:
          settings.affiliate_percent !== undefined
            ? settings.affiliate_percent
            : current.affiliatePercent,
        freeShippingProgram:
          settings.free_shipping_program !== undefined
            ? settings.free_shipping_program === "true"
            : current.freeShippingProgram,
      }));
    } catch (error) {
      logError(error);
    }
  ...
  const comparisons = [
    ...
    compareMarketplace("Shein"),
    compareMarketplace("Temu"),
    compareMarketplace("Kwai Shop"),
  ];
